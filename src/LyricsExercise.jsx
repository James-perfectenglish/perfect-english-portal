...
function fuzzyMatch(guess, target) {
  const g = guess.trim().toLowerCase(); const t = target.trim().toLowerCase();
  if (!g) return false; if (g === t) return true;
...
function parseJsonField(val) {
  if (Array.isArray(val)) return val; try { return JSON.parse(val || "[]"); } catch { return []; }
}
...
function PrimaryBtn({ children, onClick, style = {} }) {
  return (
    <button onClick={onClick} style={{ width: "100%", padding: "1rem", marginTop: "0.75rem", fontSize: "1rem", background: GRADIENT, color: "white", border: "none", borderRadius: "10px", cursor: "pointer", fontWeight: "600", fontFamily: "inherit", ...style }}>
      {children}
    </button>
  );
}
...
function ResultPill({ ok, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 14px", borderRadius: "8px", background: ok ? "#f0fff4" : "#fff5f5", border: `1px solid ${ok ? "#9ae6b4" : "#feb2b2"}`, color: ok ? "#276749" : "#c53030", marginBottom: "8px", fontSize: "0.95rem", fontWeight: "500" }}>
      <span>{ok ? "✅" : "❌"}</span><span>{label}</span>
    </div>
  );
}

const inputStyle = { width: "100%", padding: "0.9rem 1rem", fontSize: "clamp(1rem, 3.5vw, 1.15rem)", borderRadius: "8px", border: "2px solid #667eea", boxSizing: "border-box", color: "#2d3748", fontWeight: 500, backgroundColor: "#EDE9FE", fontFamily: "inherit", outline: "none", marginBottom: "0.75rem" };
...
export default function LyricsExercise({ user }) {
  ...
  async function startMode(m) {
    ...
    if (err || !data?.length) { setError("Couldn't load songs. Please try again."); setLoading(false); return; }
    const picked = shuffle(data).slice(0, TOTAL);
    setMode(m); setExercises(picked); setIdx(0); setHistory([]);
    buildPool(picked[0], m); resetRound();
    setLoading(false); setPhase("play");
  }
  ...
  function resetRound() {
    setLyricsFeedback(null); setLyricsOk(false);
  ...
  function handleLyricsResult(isCorrect, isSoft) {
    ...
    if (isCorrect) {
      ...
      setLyricsFeedback({ correct: true, message: isSoft ? `✅ Correct! (minor punctuation — still counts) ${ptLabel}` : `✅ Correct! ${ptLabel}` });
    } else {
      const correctLine = parseJsonField(ex?.words).join(" ");
      setLyricsFeedback({ correct: false, message: `❌ Not quite. The correct lyric is: "${correctLine}"` });
    }
  }
  ...
  async function finishRound(lo, so, bo) {
    ...
    await supabase.from("lyrics_sessions").insert({ student_id: user.id, exercise_id: ex.id, difficulty: mode, lyrics_correct: lo, guessed_song: so, guessed_band: bo, score });
    setPhase("result");
  }
  ...
  async function handleNext() {
    ...
    if (idx + 1 < exercises.length) {
      const ni = idx + 1; setIdx(ni); buildPool(exercises[ni], mode); resetRound(); setPhase("play");
    } else { await finaliseLeaderboard(); setPhase("board"); }
  }

  async function finaliseLeaderboard() {
    const sessionTotal = history.reduce((s, r) => s + r.score, 0);
    const initials = profile?.full_name ? profile.full_name.trim().split(/\s+/).map(n => n[0].toUpperCase()).slice(0, 2).join("") : "??";
    const { data: existing } = await supabase.from("lyrics_leaderboard").select("total_score, sessions_count").eq("student_id", user.id).maybeSingle();
    await supabase.from("lyrics_leaderboard").upsert({ student_id: user.id, initials, total_score: (existing?.total_score || 0) + sessionTotal, sessions_count: (existing?.sessions_count || 0) + 1, updated_at: new Date().toISOString() }, { onConflict: "student_id" });
    const { data: lb } = await supabase.from("lyrics_leaderboard").select("*").order("total_score", { ascending: false }).limit(10);
    setLeaderboard(lb || []);
  }
      ...
      <div style={{ backgroundColor: "#f8f9fa", minHeight: "100vh" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "1rem" }}>
          ...
          <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
            ...
            {error && <div style={{ color: "#c53030", background: "#fff5f5", borderRadius: "8px", padding: "12px 16px", marginBottom: "16px", border: "1px solid #feb2b2" }}>{error}</div>}
  ...
  if (phase === "play") {
    ...
    return (
      <div style={{ backgroundColor: "#f8f9fa", minHeight: "100vh" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "1rem" }}>
          <PageHeader sub={`Round ${idx + 1} of ${exercises.length} · ${cfg.label}`} badge={cfg.label} />
          <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
            ...
            {cfg.showArtist && ex && (
              <div style={{ background: "#f0f4ff", border: "2px solid #c7d2fe", borderRadius: "10px", padding: "1rem 1.25rem", marginBottom: "1.25rem", textAlign: "center" }}>
                <div style={{ fontSize: "0.75rem", color: "#6366f1", fontWeight: 700, letterSpacing: "0.05em", marginBottom: "4px", textTransform: "uppercase" }}>🎤 Artist</div>
                <div style={{ fontSize: "1.4rem", fontWeight: 800, color: "#1e293b" }}>{ex.artist}</div>
              </div>
            )}
            <SentenceBuildingInput
              key={`${mode}-${idx}`} words={wordPool} questionType="build"
              prompt="👆 Rearrange the words to recreate the lyric"
              correctSentences={[correctLine]} explanation="" disabled={!!lyricsFeedback}
              onResult={handleLyricsResult} feedback={lyricsFeedback} showCheckButton={true}
              allWordsUsed={!cfg.distractors} language="en" showTypeBadge={false}
            />
            {lyricsFeedback && (
              <button onClick={() => setPhase("song")} style={{ width: "100%", padding: "1rem", marginTop: "0.75rem", fontSize: "1rem", fontWeight: "600", background: GRADIENT, color: "white", border: "none", borderRadius: "10px", cursor: "pointer", fontFamily: "inherit" }}>
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "1rem" }}>
          ...
          <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
            <div style={{ border: `2px solid ${lyricsOk ? "#9ae6b4" : "#feb2b2"}`, borderRadius: "10px", marginBottom: "1.25rem", overflow: "hidden" }}>
              <div style={{ background: lyricsOk ? "#48bb78" : "#f56565", color: "white", padding: "0.6rem 1rem", fontWeight: 700, fontSize: "0.95rem" }}>Lyric</div>
            ...
            <div style={{ border: "2px solid #e2e8f0", borderRadius: "10px", overflow: "hidden", marginBottom: "1rem" }}>
              ...
              <div style={{ padding: "1rem" }}>
                {!songDone ? (
                  <>
                    <input style={inputStyle} type="text" value={songGuess} onChange={e => setSongGuess(e.target.value)} onKeyDown={e => e.key === "Enter" && submitSongGuess()} placeholder="Song title..." autoFocus />
                  ...
                  <>
                    ...
                    <PrimaryBtn onClick={proceedFromSong}>{mode === "easy" ? "See my score →" : "Next →"}</PrimaryBtn>
                  </>
                )}
              </div>
            </div>
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "1rem" }}>
          ...
          <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
            <div style={{ background: "#f7fafc", border: "2px solid #e2e8f0", borderRadius: "10px", padding: "1rem", textAlign: "center", marginBottom: "1.25rem" }}>
              ...
              <div style={{ fontSize: "2.5rem", fontWeight: 800, color: "#667eea", lineHeight: 1.2 }}>{ptsEarned} <span style={{ fontSize: "1.2rem", color: "#a0aec0" }}>/ {cfg.maxPts}</span></div>
            </div>
            <div style={{ border: "2px solid #e2e8f0", borderRadius: "10px", overflow: "hidden" }}>
              ...
              <div style={{ padding: "1rem" }}>
                {!bandDone ? (
                  <>
                    <input style={inputStyle} type="text" value={bandGuess} onChange={e => setBandGuess(e.target.value)} onKeyDown={e => e.key === "Enter" && submitBandGuess()} placeholder="Artist or band name..." autoFocus />
                    <div style={{ display: "flex", gap: "10px" }}>
    ...
    return (
      <div style={{ backgroundColor: "#f8f9fa", minHeight: "100vh" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "1rem" }}>
          <PageHeader sub={`Round ${idx + 1} of ${exercises.length} · ${cfg.label}`} badge={cfg.label} />
          <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
            <p style={{ color: "#718096", fontSize: "0.9rem", fontStyle: "italic", margin: "0 0 1rem" }}>{last?.exercise?.artist} — "{last?.exercise?.song_title}"</p>
            <ResultPill ok={last?.lyricsOk} label={`Lyric order`} />
            <ResultPill ok={last?.songOk} label={`Song: "${last?.exercise?.song_title}"`} />
            {mode !== "easy" && <ResultPill ok={last?.bandOk} label={`Artist: ${last?.exercise?.artist}`} />}
            <div style={{ background: "#f7fafc", border: "2px solid #e2e8f0", borderRadius: "10px", padding: "1.25rem", textAlign: "center", marginTop: "1.25rem" }}>
              ...
              <div style={{ fontSize: "3rem", fontWeight: 800, color: "#667eea", lineHeight: 1.2 }}>{last?.score} <span style={{ fontSize: "1.4rem", color: "#a0aec0" }}>/ {cfg.maxPts}</span></div>
              <div style={{ fontSize: "0.9rem", color: "#718096", marginTop: "4px" }}>Running total: <strong style={{ color: "#667eea" }}>{runningTotal}</strong> pt{runningTotal !== 1 ? "s" : ""} from {history.length} round{history.length !== 1 ? "s" : ""}</div>
            </div>
            <PrimaryBtn onClick={handleNext}>{isLast ? "See leaderboard 🏆" : "Next song →"}</PrimaryBtn>
          </div>
        </div>
      </div>
    );
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "1rem" }}>
          ...
          <div style={{ background: "white", padding: "2rem", borderRadius: "12px", boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
            <div style={{ background: "#f7fafc", border: "2px solid #e2e8f0", borderRadius: "10px", padding: "1.25rem", textAlign: "center", marginBottom: "1.5rem" }}>
              <div style={{ fontSize: "0.8rem", color: "#718096", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px" }}>This session · {cfg.label}</div>
              <div style={{ fontSize: "3rem", fontWeight: 800, color: "#667eea", lineHeight: 1.2, margin: "4px 0" }}>{sessionTotal} <span style={{ fontSize: "1.4rem", color: "#a0aec0" }}>/ {maxPossible}</span></div>
              <div style={{ fontSize: "1.4rem", marginTop: "4px" }}>{history.map((r, i) => <span key={i} title={r.exercise?.song_title}>{r.lyricsOk ? "✅" : "❌"}</span>)}</div>
            </div>
              ...
              leaderboard.map((row, i) => {
                ...
                return (
                  <div key={row.student_id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 8px", borderRadius: "8px", borderBottom: i < leaderboard.length - 1 ? "1px solid #f1f5f9" : "none", background: isMe ? "#f0f4ff" : "transparent" }}>
                    <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: i === 0 ? "#fbbf24" : i === 1 ? "#94a3b8" : i === 2 ? "#b45309" : "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 700, color: i < 3 ? "white" : "#64748b", flexShrink: 0 }}>{i + 1}</div>
                    <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: GRADIENT, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: "14px", flexShrink: 0 }}>{row.initials}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: "0.95rem", color: "#1e293b" }}>{row.initials}{isMe && <span style={{ fontSize: "0.75rem", color: "#667eea" }}> · you</span>}</div>
                      <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>{row.sessions_count} session{row.sessions_count !== 1 ? "s" : ""}</div>
                    </div>
            ...
            <PrimaryBtn onClick={() => setPhase("pick")} style={{ marginTop: "1.5rem" }}>Play again 🎵</PrimaryBtn>
          </div>
        </div>
      </div>
    );
