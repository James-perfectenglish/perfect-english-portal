...
const GRADIENT = 'linear-gradient(135deg, #38b2ac 0%, #2c7a7b 100%)';
...
export default function OddOneOut({ onBack, onComplete, topicFilter }) {
  ...
  const handleSelect = (option) => {
    ...
    if (isCorrect) {
      ...
      setFeedback({
        correct: true,
        oddOne,
        message: `✅ Correct! "${oddOne}" is the odd one out. ${q.explanation || ''}`
      });
    } else {
      setFeedback({
        correct: false,
        oddOne,
        message: `❌ Not quite. "${oddOne}" is the odd one out. ${q.explanation || ''}`
      });
    }
  };
  ...
  const getOptionStyle = (option) => {
    ...
    if (!feedback) {
      if (selected === option) {
        return { ...base, borderColor: '#38b2ac', backgroundColor: '#e6fffa', color: '#234e52' };
      }
      return base;
    }
    ...
    if (isOdd && feedback.correct) {
      return { ...base, borderColor: '#48bb78', backgroundColor: '#f0fff4', color: '#276749', boxShadow: '0 0 0 3px rgba(72, 187, 120, 0.3)' };
    }
    if (isOdd && !feedback.correct) {
      return { ...base, borderColor: '#48bb78', backgroundColor: '#f0fff4', color: '#276749', boxShadow: '0 0 0 3px rgba(72, 187, 120, 0.3)' };
    }
    if (wasSelected && !feedback.correct) {
      return { ...base, borderColor: '#f56565', backgroundColor: '#fff5f5', color: '#c53030' };
    }
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div style={{
          ...
          <p style={{ margin: '8px 0 0', opacity: 0.9 }}>
            Find the word that doesn't belong in each group
          </p>
        </div>
        ...
        {stage === 'playing' && q && (
          <>
            ...
            <div style={{
              ...
              {/* Question / hint text */}
              <div style={{
                fontSize: 'clamp(1.05rem, 3.5vw, 1.2rem)', color: '#2d3748',
                fontWeight: '500', marginBottom: '1.25rem', lineHeight: 1.5
              }}>
                {q.question && q.question.trim() !== ''
                  ? q.question
                  : 'Which one doesn\'t belong? Tap the odd one out.'}
              </div>
        ...
        {stage === 'finished' && (
          <div style={{
            ...
            <p style={{ color: '#4a5568' }}>
              {score >= 9 ? 'Outstanding! You really know your word groups.'
                : score >= 7 ? 'Great work! Strong vocabulary knowledge.'
                : score >= 5 ? 'Good effort. Keep building your vocabulary.'
                : 'Keep going — practice makes perfect!'}
            </p>
            <div style={{ marginTop: '20px', display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={restartExercise} style={{
                padding: '10px 24px', background: '#38b2ac', color: 'white',
