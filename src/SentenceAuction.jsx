...
// Level tier mapping — determines leaderboard eligibility
...
// A student's score counts if they're playing AT or ABOVE their home tier
...
export default function SentenceAuction({ onBack, onComplete }) {
  ...
  const saveLeaderboardScore = async (finalBudget, auctionsCompleted) => {
    ...
    try {
      ...
      const valid = isValidForLeaderboard(studentLevel, selectedLevel.key);
      if (!valid) return;
      ...
      await supabase.from('auction_scores').insert({
        ...
        auction_tier: selectedLevel.key,
  ...
  const fetchLeaderboard = async () => {
    try {
      const { data } = await supabase
        .from('auction_scores')
        .select('*')
        .order('final_budget', { ascending: false })
        .limit(100);
      if (!data) return;
      // Best score per student
      const best = {};
      data.forEach(row => {
        if (!best[row.student_id] || row.final_budget > best[row.student_id].final_budget) {
          best[row.student_id] = row;
        }
      });
      const top10 = Object.values(best)
        .sort((a, b) => b.final_budget - a.final_budget)
        .slice(0, 10);
  ...
  const nextAuction = () => {
    ...
    if (isBust) {
      ...
      saveLeaderboardScore(0, auctionsCompleted);
      fetchLeaderboard();
      setStage('finished');
      return;
    }
  ...
  const validForBoard = userProfile && selectedLevel
    ? isValidForLeaderboard(userProfile.level || 'A1', selectedLevel.key)
    : false;
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', padding: '2rem', borderRadius: '0 0 12px 12px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
          ...
          <div style={{ background: '#f7f8ff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '1rem 1.25rem', marginBottom: '24px', fontSize: '0.9rem', color: '#4a5568', lineHeight: 1.7 }}>
            <strong style={{ color: '#2d3748' }}>How to play:</strong> You start with <strong>1,000 points</strong>. Each auction shows several sentences — some correct, some not. Bid on the ones you think are right. Correct bid → win that amount. Wrong bid → lose it. Keep going until you run out of money — or questions! 🎰
          </div>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                      ...
                      {playerLevel && (
                        <span style={{ fontSize: '0.78rem', color: valid ? '#276749' : '#a0aec0', fontWeight: 500 }}>
                          {valid ? '🏆 Leaderboard eligible' : '👁 Practice only'}
                        </span>
                      )}
                    </div>
        ...
        <div style={{ background: 'white', padding: '1.5rem 2rem 2rem', borderRadius: '0 0 12px 12px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '1.15rem', fontWeight: 700, color: getBudgetColour() }}>
          ...
          {stage === 'bidding' && (
            <div style={{ background: '#f7f8ff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
              <span style={{ fontSize: '0.9rem', color: '#4a5568' }}>Bids placed: <strong>{totalBid.toLocaleString()} pts</strong></span>
              <span style={{ fontSize: '0.9rem', color: '#4a5568' }}>Available: <strong style={{ color: remainingBudget === 0 ? '#f56565' : '#667eea' }}>{remainingBudget.toLocaleString()} pts</strong></span>
            </div>
          )}
          ...
          {stage === 'bidding' && (
            <p style={{ fontSize: '0.9rem', color: '#718096', margin: '0 0 16px', lineHeight: 1.5 }}>
              Which sentences are correct? Set your bids, then lock them in.
            </p>
          )}
          {stage === 'revealing' && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '24px' }}>
            {sentences.map((s, idx) => {
              ...
              const lostBid = isRevealed && bid > 0 && !s.correct;
              ...
              if (isRevealed) {
                ...
                cardBg = '#EDE9FE'; cardBorder = '2px solid #667eea';
              }
            ...
            <div>
              ...
              <div style={{ background: auctionResult.net >= 0 ? '#f0fff4' : '#fff5f5', border: `2px solid ${auctionResult.net >= 0 ? '#48bb78' : '#f56565'}`, borderRadius: '12px', padding: '1.25rem', marginBottom: '16px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.9rem', color: '#4a5568' }}>
                  Won: <strong style={{ color: '#276749' }}>+{auctionResult.gained}</strong> · Lost: <strong style={{ color: '#c53030' }}>-{auctionResult.lost}</strong>
                  {!isBust && <> · Budget: <strong style={{ color: getBudgetColour() }}>{budget.toLocaleString()}</strong></>}
                </div>
              </div>
        ...
        <div style={{ background: 'white', padding: '2rem', borderRadius: '0 0 12px 12px', boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '24px' }}>
            ...
            {validForBoard && scoreSaved && (
              <div style={{ marginTop: '10px', fontSize: '0.85rem', color: '#667eea', fontWeight: 500 }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {leaderboard.map((entry, i) => {
                  const levelColour = LEVEL_COLOURS[entry.student_level] || { bg: '#e2e8f0', text: '#4a5568' };
                  ...
                  return (
                    <div key={entry.id} style={{
                      ...
                      background: isCurrentUser ? '#f7f8ff' : '#f9fafb',
                      border: isCurrentUser ? '2px solid #667eea' : '1px solid #e2e8f0',
                    }}>
                      {/* Rank */}
                      ...
                      {/* Student level badge */}
                      <span style={{ background: levelColour.bg, color: levelColour.text, padding: '2px 8px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, flexShrink: 0 }}>
                        {entry.student_level}
                      </span>
                      {/* Auction tier badge */}
                      {tierLevel && (
                        <span style={{ background: tierLevel.colour + '30', color: tierLevel.colour, padding: '2px 8px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, flexShrink: 0, border: `1px solid ${tierLevel.colour}60` }}>
                          {tierLevel.label}
                        </span>
                      )}
                      {/* Auctions */}
                      ...
                      <div style={{ marginLeft: 'auto', fontWeight: 700, fontSize: '1.05rem', color: entry.final_budget >= STARTING_BUDGET ? '#276749' : entry.final_budget > 0 ? '#c05621' : '#c53030' }}>
                        {entry.final_budget.toLocaleString()} pts
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {roundResults.map((r, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderRadius: '8px', background: '#f7fafc', border: '1px solid #e2e8f0' }}>
                    ...
                    <span style={{ fontWeight: 700, color: r.net >= 0 ? '#276749' : '#c53030', fontSize: '0.95rem' }}>{r.net >= 0 ? '+' : ''}{r.net} pts</span>
                  </div>
                ))}
              </div>
            </div>
