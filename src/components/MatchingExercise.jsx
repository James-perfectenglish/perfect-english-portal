...
export default function MatchingExercise({ onBack, onComplete, topicFilter }) {
  ...
  // Set by MatchingPairs via onResult when all pairs are matched
  const [questionResult, setQuestionResult] = useState(null); // { isCorrect, wrongAttempts }

  useEffect(() => { fetchCounts(); }, []);

  const fetchCounts = async () => {
    let query = supabase.from('question_bank').select('level').eq('type', 'matching');
  ...
  const fetchQuestions = async (dbLevels) => {
    ...
    if (data && data.length > 0) {
      setQuestions(shuffleArray(data).slice(0, QUESTIONS_PER_ROUND));
    } else {
      setQuestions([]);
    }
    setCurrentQ(0);
    ...
    return (
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div style={{
          ...
          padding: '2.5rem 2rem 2rem', textAlign: 'center', color: 'white',
        }}>
          <h1 style={{ margin: 0, fontSize: '1.8rem' }}>🔗 Matching</h1>
          <p style={{ margin: '8px 0 0', opacity: 0.9 }}>
            Match the pairs — translations, definitions, and sounds
          </p>
        </div>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <span style={{
                      ...
                      fontSize: '0.8rem', color: available ? '#4a5568' : '#a0aec0', fontWeight: 500,
                    }}>
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <h1 style={{ margin: 0, fontSize: '1.8rem' }}>🔗 Matching</h1>
        <p style={{ margin: '8px 0 0', opacity: 0.9 }}>
          Tap a tile on the left, then its match on the right
        </p>
        {selectedLevel && (
          <span style={{
            display: 'inline-block',
            background: selectedLevel.colour,
      ...
      <div style={{
        ...
        {stage === 'playing' && q && (
          <>
            ...
            <div style={{
              ...
              background: '#f7fafc', padding: '12px 16px', borderRadius: '8px',
              marginBottom: '20px', fontSize: '0.9rem', color: '#4a5568', fontWeight: 500,
            }}>
            ...
            {q.question && q.question.trim() && (
              <div style={{
                fontSize: 'clamp(1rem, 3.5vw, 1.15rem)', color: '#2d3748',
            ...
            {/* The matching grid — key forces re-mount between questions */}
            <MatchingPairs
              key={currentQ}
              pairs={parsedPairs}
            ...
            {/* Feedback after all matched */}
            {questionResult && (
              <div style={{ marginTop: '16px' }}>
                <div style={{
                  backgroundColor: questionResult.isCorrect ? '#f0fff4' : '#fff3cd',
                  border: `1px solid ${questionResult.isCorrect ? '#c6f6d5' : '#feebc8'}`,
                  color: questionResult.isCorrect ? '#276749' : '#856404',
                  ...
                  {q.explanation && (
                    <div style={{ marginTop: '8px', opacity: 0.85 }}>{q.explanation}</div>
                  )}
                </div>
        ...
        {stage === 'finished' && (
          <div style={{
            ...
            borderRadius: '8px', padding: '2rem', textAlign: 'center', marginTop: '1rem',
          }}>
            ...
            <div style={{
              ...
              color: score >= questions.length * 0.7 ? '#48bb78' : score >= questions.length * 0.5 ? '#ed8936' : '#f56565',
            }}>
              {score} / {questions.length}
            </div>
            <p style={{ color: '#4a5568' }}>
              {score >= questions.length * 0.9
                ? 'Outstanding! Flawless matching — you really know your vocabulary connections.'
                : score >= questions.length * 0.7
                  ? 'Great work! Strong vocabulary connections.'
                  : score >= questions.length * 0.5
                    ? 'Good effort. Keep building those vocabulary links.'
                    : 'Keep going — the connections will come with practice!'}
            </p>
            <p style={{ color: '#718096', fontSize: '0.88rem' }}>
              A point is awarded for each set matched with no wrong attempts.
            </p>
