/* ============================================================
   ConditionalCard — one conditional's reference card (shared, EN)
   ------------------------------------------------------------
   Used by:
     • ConditionalExplainer.jsx — accordion mode (with onPractise)
     • ConditionalChooser.jsx   — "📖 See the full card" overlay
   Sibling of TenseCard / ModalCard (same palette + shape). No bank
   fetch — conditionals have no specimens bank; the curated examples
   and the woven-in negative row carry the card.
   ============================================================ */
...
/* tiny **bold** / *italic* renderer — same convention as the bank */
...
/* condition → result timeline. Solid = real, dashed hollow = hypothetical
   (the same visual grammar as the tense timelines' condicional).
   shape: 'zero' | 'first' | 'second' | 'third' | 'mixed' | null */
export function ConditionalTimeline({ shape }) {
  ...
  const ifLabel = (x) => <text x={x - 4} y={mid - 10} fontSize="8" fill={C.brandDark} fontWeight="700">if</text>;
  let shapes = null;
  if (shape === 'zero') {
    /* timeless: a band across the whole line — true whenever it happens */
    shapes = <rect x="14" y={mid - 5} width={W - 28} height="10" rx="5" fill={C.band} opacity="0.7" />;
  } else if (shape === 'first') {
    shapes = <g>{ifLabel(176)}{arrow(176, 226, false)}{solidDot(176)}{solidDot(226)}</g>;
  } else if (shape === 'second') {
    shapes = <g>{ifLabel(176)}{arrow(176, 226, true)}{hollowDot(176)}{hollowDot(226)}</g>;
  } else if (shape === 'third') {
    shapes = <g>{ifLabel(56)}{arrow(56, 108, true)}{hollowDot(56)}{hollowDot(108)}</g>;
  } else if (shape === 'mixed') {
    shapes = <g>{ifLabel(62)}{arrow(62, nowX, true)}{hollowDot(62)}{hollowDot(nowX)}</g>;
  }
  return (
...
export default function ConditionalCard({ card, onPractise }) {
  if (!card) return null;
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
      ...
      <div style={{ padding: '1.1rem' }}>
        {card.timeline && (
          <div style={{ marginBottom: '1rem' }}><ConditionalTimeline shape={card.timeline} /></div>
        )}
