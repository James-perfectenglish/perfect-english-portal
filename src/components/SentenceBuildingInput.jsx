...
function normalizeAnswer(words) { let result = words.map(w => w.text).join(' '); result = result.replace(/ ([.,?!;:])/g, '$1'); return result.toLowerCase().trim(); }
...
const aiMarkSentence = async (correctAnswer, studentAnswer, language = 'en') => {
  try {
    ...
    return result;
  } catch (e) { console.error('AI sentence marking error:', e); return null; }
};
 ...
 * - showTypeBadge: boolean — when false, hides the type badge (e.g. "Build a sentence")
 */
export default function SentenceBuildingInput({
  ...
  useEffect(() => { if (onAnswerReady) onAnswerReady(answerWords.length > 0); }, [answerWords.length]);

  useEffect(() => { if (getCheckFn) { getCheckFn(() => checkAnswer()); } }, [answerWords, correctSentences]);
  ...
  const resetWords = () => {
    const wordsWithIds = words.map((text, i) => ({ id: `w${i}`, text }));
    setBankWords(shuffleArray(wordsWithIds));
    setAnswerWords([]); setDropIndex(null); setDragWord(null); setDragPos(null);
  };
  ...
  const calcDropIndex = (cursorX, cursorY) => {
    ...
    for (let i = 0; i < refs.length; i++) {
      const el = refs[i]; if (!el) continue;
      ...
      if (onSameRow) { if (cursorX < r.left + r.width / 2) return i; }
    }
    return refs.length;
  };
  ...
  const handlePointerDown = useCallback((word, zone, e) => {
    ...
    const handleMove = (moveE) => {
      ...
      if (!isDragging.current && Math.sqrt(dx * dx + dy * dy) > DRAG_THRESHOLD) { isDragging.current = true; setDragWord(pendingWord); }
      if (isDragging.current) {
        moveE.preventDefault();
        setDragPos({ x: t.clientX, y: t.clientY });
        if (answerZoneRef.current) {
          const zr = answerZoneRef.current.getBoundingClientRect();
          if (t.clientY >= zr.top - 50 && t.clientY <= zr.bottom + 50) { setDropIndex(calcDropIndex(t.clientX, t.clientY)); }
    ...
    const handleUp = () => {
      ...
      if (!isDragging.current) {
        ...
        if (zone === 'bank') { setBankWords(prev => prev.filter(w => w.id !== word.id)); setAnswerWords(prev => [...prev, word]); }
        else { setAnswerWords(prev => prev.filter(w => w.id !== word.id)); setBankWords(prev => [...prev, word]); }
      } else {
        setDragWord(null); setDragPos(null); isDragging.current = false;
        if (zone === 'bank') {
          setDropIndex(currentIdx => {
            setBankWords(prev => prev.filter(w => w.id !== word.id));
            setAnswerWords(prev => { const idx = currentIdx !== null && currentIdx !== undefined ? currentIdx : prev.length; const newAnswer = [...prev]; newAnswer.splice(idx, 0, word); return newAnswer; });
            return null;
          });
        } else {
          setDropIndex(currentIdx => {
            if (currentIdx !== null && currentIdx !== undefined) {
              setAnswerWords(prev => { const filtered = prev.filter(w => w.id !== word.id); filtered.splice(Math.min(currentIdx, filtered.length), 0, word); return filtered; });
            } else { setAnswerWords(prev => prev.filter(w => w.id !== word.id)); setBankWords(prev => [...prev, word]); }
            return null;
          });
        }
  ...
  const tileStyle = (isDragSource = false) => ({
    ...
    padding: 'clamp(8px, 2.5vw, 10px) clamp(12px, 3vw, 16px)', margin: 'clamp(4px, 1.5vw, 6px)',
  ...
  const dropIndicator = (
    <span style={{ display: 'inline-block', width: '3px', height: '36px', backgroundColor: '#667eea', borderRadius: '2px', margin: '4px 2px', verticalAlign: 'middle', animation: 'sbPulse 0.8s ease-in-out infinite alternate' }} />
  );
    ...
    <div>
      ...
      <div style={{ marginBottom: '1.25rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
          <div style={{ fontSize: '0.8rem', fontWeight: '600', color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Your answer</div>
          {targetTileCount && (
        ...
        <div ref={answerZoneRef} style={{ minHeight: '70px', border: answerWords.length === 0 ? '2px dashed #AED6F1' : '2px solid #AED6F1', borderRadius: '12px', padding: '12px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', alignContent: 'flex-start', backgroundColor: '#F8FBFF' }}>
          {answerWords.length === 0 && !dragWord && (
            <div style={{ width: '100%', textAlign: 'center', color: '#94a3b8', fontSize: 'clamp(0.85rem, 2.5vw, 0.95rem)', padding: '0.5rem 0' }}>Tap or drag words here to build your sentence</div>
          )}
          {answerWords.map((word, index) => {
            ...
            return (
              <span key={word.id}>
                ...
                <span ref={el => answerRefs.current[index] = el} onMouseDown={(e) => handlePointerDown(word, 'answer', e)} onTouchStart={(e) => handlePointerDown(word, 'answer', e)} style={answerTileStyle(isDragSource)}>{word.text}</span>
              </span>
            );
          })}
      ...
      <div style={{ marginBottom: '1.25rem' }}>
        <div style={{ fontSize: '0.8rem', fontWeight: '600', color: '#666', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          Word bank
          {!allWordsUsed && <span style={{ fontWeight: '400', textTransform: 'none', letterSpacing: 'normal', color: '#999' }}>{' '}— you won't need all the words</span>}
        </div>
        <div style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '12px', display: 'flex', flexWrap: 'wrap', minHeight: '50px', backgroundColor: '#fafafa' }}>
          {bankWords.map(word => {
            ...
            return (<span key={word.id} onMouseDown={(e) => handlePointerDown(word, 'bank', e)} onTouchStart={(e) => handlePointerDown(word, 'bank', e)} style={tileStyle(isDragSource)}>{word.text}</span>);
          })}
          {bankWords.length === 0 && <div style={{ width: '100%', textAlign: 'center', color: '#cbd5e0', fontSize: '0.85rem' }}>All words placed</div>}
        </div>
      </div>
      ...
      {showCheckButton && !feedback && (
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          <button onClick={checkAnswer} disabled={answerWords.length === 0 || isChecking} style={{ flex: 1, padding: '1rem', fontSize: 'clamp(1rem, 3.5vw, 1.15rem)', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', opacity: (answerWords.length === 0 || isChecking) ? 0.5 : 1, minWidth: '120px' }}>
            {isChecking ? '🤖 Checking...' : 'Check Answer'}
          </button>
          <button onClick={resetWords} style={{ padding: '1rem 1.25rem', fontSize: 'clamp(1rem, 3.5vw, 1.15rem)', backgroundColor: 'transparent', color: '#666', border: '1px solid #ddd', borderRadius: '10px', cursor: 'pointer', fontWeight: '500' }}>🔄 Reset</button>
        </div>
      )}
      {!showCheckButton && !feedback && (
        <button onClick={resetWords} style={{ padding: '0.5rem 1rem', fontSize: 'clamp(0.85rem, 2.5vw, 0.9rem)', backgroundColor: 'transparent', color: '#888', border: '1px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', fontWeight: '500', marginBottom: '0.5rem' }}>🔄 Reset words</button>
      )}
