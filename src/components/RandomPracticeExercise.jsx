...
export default function RandomPracticeExercise({ levels, levelTitle, levelSubtitle, gradient, onBack }) {
  ...
  const saveAnswer = async (question, studentAnswer, isCorrect) => {
    try {
      ...
      const correctAnswer = question.type === 'multiple_choice' ? (question.correct_answer || correctAnswers[0] || '') : (correctAnswers[0] || '');
      await supabase.from('student_answers').insert({
        student_id: user.id, question_id: question.question_number,
        student_answer: studentAnswer, correct_answer: correctAnswer, is_correct: isCorrect
      });
  ...
  const startExercise = async () => {
    ...
    try {
      ...
      const [gfRes, mcRes, sbRes, oooRes, ecRes] = await Promise.all([
        queryForType('gap_fill'), queryForType('multiple_choice'), queryForType('sentence_building'),
        queryForType('odd_one_out'), queryForType('error_correction')
      ]);
  ...
  const checkAnswer = () => {
    ...
    if (currentQuestion.type === 'gap_fill') {
      ...
      const correctAnswers = Array.isArray(currentQuestion.correct_answers) ? currentQuestion.correct_answers.map(a => a.toLowerCase().trim()) : [];
      if (correctAnswers.includes(answer)) { isCorrect = true; feedbackMessage = `✅ Correct! ${currentQuestion.explanation}`; feedbackType = 'correct'; }
        ...
        if (informalAnswers.includes(answer)) { isCorrect = true; feedbackMessage = `✅ Correct! ${currentQuestion.informal_feedback || ''} ${currentQuestion.explanation}`; feedbackType = 'informal'; }
      }
      if (!isCorrect && currentQuestion.acceptable_alternatives && Array.isArray(currentQuestion.acceptable_alternatives)) {
        ...
        if (alternative) { isCorrect = true; feedbackMessage = `✅ ${alternative.feedback} ${currentQuestion.explanation}`; feedbackType = 'alternative'; }
      }
      if (!isCorrect) { feedbackMessage = `❌ Incorrect. The correct answer is: "${correctAnswers[0] || 'N/A'}". ${currentQuestion.explanation}`; }
      saveAnswer(currentQuestion, userAnswer.trim(), isCorrect);

    } else if (currentQuestion.type === 'multiple_choice') {
      if (selectedOption === currentQuestion.correct_answer) { isCorrect = true; feedbackMessage = `✅ Correct! ${currentQuestion.explanation}`; feedbackType = 'correct'; }
      else { feedbackMessage = `❌ Incorrect. The correct answer is: "${currentQuestion.correct_answer}". ${currentQuestion.explanation}`; }
  ...
  const handleOOOSelect = (option) => {
    ...
    const correctAnswers = Array.isArray(cq.correct_answers) ? cq.correct_answers : JSON.parse(cq.correct_answers || '[]');
  ...
  // EC handlers
  const handleECWordTap = (index) => { if (feedback) return; setEcSelectedWordIndex(index); setEcCorrection(''); };
  ...
  const checkECAnswer = () => {
    ...
    const correctAnswers = Array.isArray(cq.correct_answers) ? cq.correct_answers : JSON.parse(cq.correct_answers || '[]');
    const correctedWords = [...words]; correctedWords[ecSelectedWordIndex] = ecCorrection.trim();
    ...
    let feedbackMessage;
    if (isCorrect) { feedbackMessage = `✅ Correct! ${cq.explanation || ''}`; }
    else {
  ...
  const handleSentenceBuildingResult = (isCorrect, isSoft = false, userAnswer = '') => {
    ...
    if (isCorrect) {
      ...
      setSbFeedback({ correct: true, message: msg }); setFeedback({ message: msg, type: 'correct', isCorrect: true });
      setScore(s => s + 1); saveAnswer(cq, userAnswer || '(correct)', true);
    } else {
      const correctSentences = Array.isArray(cq.correct_answers) ? cq.correct_answers : JSON.parse(cq.correct_answers || '[]');
      ...
      setSbFeedback({ correct: false, message: msg }); setFeedback({ message: msg, type: 'incorrect', isCorrect: false });
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      setUserAnswer(''); setSelectedOption(null); setFeedback(null); setSbFeedback(null);
      setOooSelected(null); setEcSelectedWordIndex(null); setEcCorrection(''); setShowHint(false);
    } else { finishExercise(); }
  };
  ...
  const getOOOStyle = (option) => {
    const base = {
      padding: 'clamp(8px, 2.5vw, 10px) clamp(12px, 3vw, 16px)', borderRadius: '8px',
      border: 'none', boxShadow: 'inset 0 0 0 2px #e2e8f0',
      cursor: feedback ? 'default' : 'pointer', fontSize: 'clamp(0.9rem, 3.2vw, 1.1rem)',
      fontWeight: '500', textAlign: 'center', transition: 'all 0.2s ease',
      backgroundColor: 'white', color: '#2d3748', minHeight: '55px', display: 'flex',
      alignItems: 'center', justifyContent: 'center', userSelect: 'none', outline: 'none'
    };
    if (!feedback) {
  ...
  const getECTileStyle = (index) => {
    const base = {
      ...
      padding: 'clamp(8px, 2.5vw, 10px) clamp(12px, 3vw, 16px)', margin: '4px 3px',
      borderRadius: '8px', fontSize: 'clamp(0.9rem, 3.2vw, 1.1rem)', fontWeight: '500',
      cursor: feedback ? 'default' : 'pointer', transition: 'all 0.15s ease', userSelect: 'none',
  ...
  const ecWords = currentQuestion && currentQuestion.type === 'error_correction' ? currentQuestion.question.trim().split(/\s+/) : [];
      ...
      <div style={{ padding: '1rem', width: '100%', boxSizing: 'border-box' }}>
        ...
        {stage === 'start' && (
          <div style={{ textAlign: 'center', padding: '2rem 0', minHeight: '60vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', width: '100%', boxSizing: 'border-box', maxWidth: '600px', margin: '0 auto' }}>
            ...
            <p style={{ fontSize: 'clamp(1.1rem, 4vw, 1.3rem)', color: '#2C3E50', marginBottom: '1rem', lineHeight: '1.5' }}>Test your English with 20 random questions!</p>
            <p style={{ fontSize: 'clamp(0.95rem, 3vw, 1.05rem)', color: '#666', marginBottom: '2.5rem', lineHeight: '1.5' }}>A mix of multiple choice, gap fill, sentence building, odd one out, and error correction. Answer all questions and see your score at the end.</p>
            <button onClick={startExercise} disabled={loading} style={{ padding: '1.25rem', fontSize: 'clamp(1.1rem, 4vw, 1.3rem)', background: displayGradient, color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', width: '100%', maxWidth: '350px', margin: '0 auto', fontWeight: '600', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>{loading ? 'Loading...' : 'Start Practice'}</button>
            {onBack && (
              <button onClick={onBack} style={{ marginTop: '1.5rem', padding: '0.75rem 1.5rem', fontSize: 'clamp(0.9rem, 3vw, 1rem)', backgroundColor: 'transparent', color: '#666', border: '1px solid #ddd', borderRadius: '8px', cursor: 'pointer', fontWeight: '500' }}>← Choose Different Level</button>
            )}
          </div>
        )}
          ...
          <div style={{ width: '100%', maxWidth: '700px', margin: '0 auto' }}>
            ...
            <div style={{ backgroundColor: 'white', padding: 'clamp(1.5rem, 5vw, 2.5rem)', borderRadius: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                ...
                {currentQuestion.topic && (
                  <div style={{
                    ...
                    {currentQuestion.topic === 'question_forms' ? '❓ ' : ''}{currentQuestion.topic.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
                  </div>
                )}
              </div>

              {/* Question Text — skip for SB and EC */}
              {currentQuestion.type !== 'sentence_building' && currentQuestion.type !== 'error_correction' && currentQuestion.question && (
                <div style={{ fontSize: 'clamp(1.15rem, 4vw, 1.4rem)', color: '#2C3E50', lineHeight: '1.6', fontWeight: '500', wordWrap: 'break-word', overflowWrap: 'break-word' }}>{currentQuestion.question}</div>
              )}

              {/* Hint */}
              {showHint && currentQuestion.hint && (
                <div style={{ backgroundColor: '#fff3cd', padding: '0.8rem 1rem', borderRadius: '8px', border: '1px solid #ffc107', fontSize: 'clamp(0.9rem, 3vw, 1rem)', color: '#856404' }}>💡 <strong>Hint:</strong> {currentQuestion.hint}</div>
              )}

              <div style={{ flex: 1 }}>
                {/* GAP FILL */}
                {currentQuestion.type === 'gap_fill' && !feedback && (
                  <input type="text" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && checkAnswer()} placeholder="Type your answer..." style={{ width: '100%', padding: '1.2rem', fontSize: 'clamp(1.1rem, 4vw, 1.3rem)', borderRadius: '10px', border: '2px solid #e0e0e0', boxSizing: 'border-box', color: '#2C3E50' }} autoFocus />
                )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {currentQuestion.options.map((option, index) => (
                      <button key={index} onClick={() => setSelectedOption(option)} style={{
                        ...
                        backgroundColor: selectedOption === option ? '#3498DB' : 'white', color: selectedOption === option ? 'white' : '#2C3E50',
                        border: `2px solid ${selectedOption === option ? '#3498DB' : '#e0e0e0'}`,
                        borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s', wordWrap: 'break-word', width: '100%', boxSizing: 'border-box', fontWeight: '500'
                      }}>{option}</button>
                    ))}
                  </div>
                )}
                ...
                {currentQuestion.type === 'error_correction' && (
                  <div>
                    <div style={{ fontSize: '0.9rem', color: '#718096', marginBottom: '1rem', fontStyle: 'italic' }}>
                      {!feedback ? 'Tap the word that is wrong, then type the correction below.' : feedback.isCorrect ? 'Well done!' : 'See the correction below.'}
                    </div>
                    <div style={{ backgroundColor: '#F8FBFF', padding: '1.25rem', borderRadius: '10px', border: '1px solid #AED6F1', lineHeight: '2.4', marginBottom: '1.25rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
                      {ecWords.map((word, index) => (
                        <span key={index} className="rp-ec-tile" tabIndex={-1} onClick={() => handleECWordTap(index)} style={getECTileStyle(index)}
                          onMouseEnter={e => { if (!feedback && ecSelectedWordIndex !== index) { e.currentTarget.style.borderColor = '#667eea'; e.currentTarget.style.backgroundColor = '#f7f7ff'; } }}
                          onMouseLeave={e => { if (!feedback && ecSelectedWordIndex !== index) { e.currentTarget.style.borderColor = '#e2e8f0'; e.currentTarget.style.backgroundColor = 'white'; } }}
                        >{word}</span>
                      ))}
                    ...
                    {ecSelectedWordIndex !== null && !feedback && (
                      <div style={{ display: 'flex', gap: '10px', marginBottom: '1rem', alignItems: 'stretch' }}>
                        <div style={{ flex: 1 }}>
                          ...
                          <input type="text" value={ecCorrection} onChange={(e) => setEcCorrection(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && checkECAnswer()} placeholder="Type the correct word..." autoFocus
                        ...
                        <button onClick={checkECAnswer} disabled={!ecCorrection.trim()} style={{ padding: '0 1.5rem', background: ecCorrection.trim() ? 'linear-gradient(135deg, #667eea, #764ba2)' : '#cbd5e0', color: 'white', border: 'none', borderRadius: '8px', cursor: ecCorrection.trim() ? 'pointer' : 'not-allowed', fontWeight: 600, fontSize: '1rem', alignSelf: 'flex-end', minHeight: '48px' }}>Check</button>
                      </div>
                    )}
                    {ecSelectedWordIndex === null && !feedback && (
                ...
                {/* FEEDBACK for non-SB types */}
                {feedback && currentQuestion.type !== 'sentence_building' && (
                  <div style={{
                    backgroundColor: feedback.isCorrect ? '#d4edda' : '#f8d7da', color: feedback.isCorrect ? '#155724' : '#721c24',
                    padding: '1.2rem', borderRadius: '10px', fontSize: 'clamp(1rem, 3vw, 1.1rem)', lineHeight: '1.6',
              ...
              <div style={{ marginTop: '1.5rem' }}>
                {!feedback && currentQuestion.type !== 'sentence_building' && currentQuestion.type !== 'odd_one_out' && currentQuestion.type !== 'error_correction' && (
                  <button onClick={checkAnswer}
                    disabled={(currentQuestion.type === 'gap_fill' && !userAnswer.trim()) || (currentQuestion.type === 'multiple_choice' && !selectedOption)}
                    style={{ padding: '1.2rem', fontSize: 'clamp(1.1rem, 4vw, 1.25rem)', backgroundColor: '#2C3E50', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', width: '100%', fontWeight: '600', opacity: (currentQuestion.type === 'gap_fill' && !userAnswer.trim()) || (currentQuestion.type === 'multiple_choice' && !selectedOption) ? 0.5 : 1 }}>Check Answer</button>
                )}
                {feedback && (
        ...
        {stage === 'finished' && (
          <div style={{ width: '100%', maxWidth: '600px', margin: '2rem auto 0' }}>
            <div style={{ backgroundColor: 'white', padding: 'clamp(2rem, 6vw, 3rem)', borderRadius: '16px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', textAlign: 'center' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: bestScore !== null ? '1fr 1fr 1fr' : '1fr', gap: '1rem', marginBottom: '2rem' }}>
                ...
                {bestScore !== null && (
                  <div style={{ background: score >= bestScore ? '#f0fff4' : '#f7fafc', border: score >= bestScore ? '2px solid #48bb78' : '2px solid #e2e8f0', borderRadius: '12px', padding: '1.25rem 1rem' }}>
                    ...
                    {score >= bestScore && score > 0 && <div style={{ fontSize: '0.85rem', color: '#48bb78', fontWeight: '600', marginTop: '0.25rem' }}>{score > bestScore ? '🎉 New best!' : '🏆 Matched!'}</div>}
                  </div>
                )}
                {averageScore !== null && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <button onClick={retry} style={{ padding: '1.2rem', fontSize: 'clamp(1.1rem, 4vw, 1.25rem)', background: displayGradient, color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600' }}>Try Again</button>
                {onBack && (
                  <button onClick={onBack} style={{ padding: '1rem', fontSize: 'clamp(1rem, 3.5vw, 1.1rem)', backgroundColor: 'transparent', color: '#666', border: '1px solid #ddd', borderRadius: '10px', cursor: 'pointer', fontWeight: '500' }}>← Back to Levels</button>
                )}
              </div>
            </div>
          </div>
