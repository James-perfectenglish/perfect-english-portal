...
import { conditionalsForLevel, levelBand, BAND_ORDER, GROUP_ORDER, GROUP_LABEL } from '../lib/conditionalExplainEs.js';
import ConditionalCardES from './ConditionalCardES';

/* ============================================================
   Conditionals Explainer 📘 — standalone Learn reference (Spanish)
   For native-English learners of Spanish. Every conditional with
   its formula, uses, the negative on every card, the formation
   boxes for the subjunctive forms (tipo 2/3), and the watch-out
   traps. Content + structure come from the shared
   src/lib/conditionalExplainEs.js (the same source the Chooser's
   📖 overlay reads on the Spanish track). Default view = the
   student's band; a toggle reveals the full ladder.
   ============================================================ */
...
export default function ConditionalExplainerES({ profile, onPractise }) {
    ...
    <div style={{ width: '100%', minHeight: '80vh', background: C.page, padding: '1rem', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ color: C.muted, fontSize: '0.85rem', marginTop: 2 }}>Spanish conditionals — real, hypothetical and formal. Tap one to open it.</div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {g.items.map(c => {
                ...
                return (
                  <div key={c.id}>
                    {open ? (
                      <ConditionalCardES card={c} onPractise={onPractise} />
                    ) : (
